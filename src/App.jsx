...
function Field({label,children}){return <div style={{display:"flex",flexDirection:"column",gap:4}}><label style={{color:C.muted,fontSize:11,letterSpacing:1,textTransform:"uppercase"}}>{label}</label>{children}</div>;}
function Badge({text,color=C.accent}){return <span style={{background:color+"22",color,border:`1px solid ${color}44`,borderRadius:4,padding:"2px 8px",fontSize:11,fontWeight:600}}>{text}</span>;}
function StatCard({label,value,sub,color=C.text,icon}){return <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 20px",display:"flex",flexDirection:"column",gap:4}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:C.muted,fontSize:11,letterSpacing:2,textTransform:"uppercase"}}>{label}</span>{icon&&<span>{icon}</span>}</div><div style={{color,fontSize:26,fontWeight:700,fontFamily:"monospace"}}>{value}</div>{sub&&<div style={{color:C.muted,fontSize:11}}>{sub}</div>}</div>;}

function EquityCurve({trades,startBalance}){
  const ref=useRef(null);
  useEffect(()=>{
    const c=ref.current;if(!c)return;const ctx=c.getContext("2d");const W=c.width,H=c.height;ctx.clearRect(0,0,W,H);
    ...
    ctx.beginPath();ctx.strokeStyle=C.accent;ctx.lineWidth=2;ctx.shadowColor=C.accent;ctx.shadowBlur=8;pts.forEach((p,i)=>i===0?ctx.moveTo(tx(i),ty(p.y)):ctx.lineTo(tx(i),ty(p.y)));ctx.stroke();ctx.shadowBlur=0;
  },[trades,startBalance]);
...
function BarChart({data,label}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:6}}>{label&&<div style={{color:C.muted,fontSize:11,letterSpacing:2,textTransform:"uppercase"}}>{label}</div>}{data.map((d,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:60,fontSize:11,color:C.muted,textAlign:"right",flexShrink:0}}>{d.label}</div><div style={{flex:1,height:18,background:C.border,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${(Math.abs(d.value)/mx)*100}%`,background:d.value>=0?C.green:C.red,borderRadius:3}}></div></div><div style={{width:40,fontSize:11,color:C.text,textAlign:"right"}}>{d.extra}</div></div>)}</div>;
}

function AuthScreen({onAuth}){
  const [mode,setMode]=useState("login"),[email,setEmail]=useState(""),[password,setPassword]=useState(""),[name,setName]=useState(""),[loading,setLoading]=useState(false),[err,setErr]=useState(""),[ok,setOk]=useState("");
  async function go(){
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:16,fontFamily:"'JetBrains Mono',monospace"}}>
      ...
      <div style={{width:"100%",maxWidth:420,animation:"fi 0.4s ease"}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:32}}>
          <div style={{fontSize:17,fontWeight:700,marginBottom:22}}>{mode==="login"?"Iniciar Sesión":mode==="register"?"Crear Cuenta":"Recuperar Contraseña"}</div>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {mode!=="forgot"&&<Field label="Contraseña"><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" style={iS} onKeyDown={e=>e.key==="Enter"&&go()}/></Field>}
          </div>
          ...
          <div style={{marginTop:18,display:"flex",flexDirection:"column",gap:8,alignItems:"center"}}>
            {mode==="login"&&<><button onClick={()=>{setMode("register");setErr("");}} style={{background:"none",border:"none",color:C.accent,cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>¿No tienes cuenta? Regístrate</button><button onClick={()=>{setMode("forgot");setErr("");}} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:11,fontFamily:"inherit"}}>¿Olvidaste tu contraseña?</button></>}
...
export default function App(){
  ...
  async function save(){
    if(!form.asset||!form.date)return notify("Campos requeridos",C.red);
    ...
    await load();setForm(ef());setEditId(null);setIp(null);setModal(false);
  }

  function edit(t){setForm({...t,pnl_r:t.pnl_r??"",pnl_usd:t.pnl_usd??""});setEditId(t.id);setIp(t.image||null);setModal(true);}
  async function del(id){await supabase.from("trades").delete().eq("id",id);await load();notify("Eliminado",C.yellow);}
  ...
  const ss=SETUPS.map(s=>{const ts=ct.filter(t=>t.setup===s);const w=ts.filter(t=>t.pnl_usd>0).length;const p=ts.reduce((a,t)=>a+t.pnl_usd,0);return{setup:s,count:ts.length,wr:ts.length?(w/ts.length*100).toFixed(0):0,pnl:p};}).filter(s=>s.count>0).sort((a,x)=>x.pnl-a.pnl);
  const ds=Array(7).fill(null).map((_,i)=>{const ts=ct.filter(t=>new Date(t.date).getDay()===i);const w=ts.filter(t=>t.pnl_usd>0).length;return{label:DAYS[i],value:ts.length?parseFloat(((w/ts.length)*100).toFixed(1)):0,extra:`${ts.length}op`};});
  const as2=ASSETS.map(a=>{const ts=ct.filter(t=>t.asset===a);const p=ts.reduce((s,t)=>s+t.pnl_usd,0);return{label:a,value:parseFloat(p.toFixed(1)),extra:ts.length>0?`${ts.length}op`:""};}).filter(a=>a.extra!=="").sort((a,x)=>x.value-a.value);
  ...
  async function ai(){
    ...
    try{
      const sum={totalTrades:ct.length,winrate:wr+"%",profitFactor:pf,avgR:aR,maxDD:mDD.toFixed(2)+"%",totalPnl:tPnl.toFixed(2),topSetups:ss.slice(0,3).map(s=>({setup:s.setup,wr:s.wr+"%",pnl:s.pnl.toFixed(0)})),avgEmotion:ae,ruleFollowPct:rp+"%",topErrors:te.map(([e,c])=>e+" x"+c)};
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:"Eres un coach de trading profesional. Analiza los datos y da recomendaciones concretas en español. Usa emojis. Estructura: 1) Resumen 2) 3 fortalezas 3) 3 áreas de mejora con acciones 4) Recomendación psicológica 5) Plan para la próxima semana.",messages:[{role:"user",content:`Analiza mi bitácora:\n${JSON.stringify(sum,null,2)}`}]})});
      const d=await r.json();setAiMsg(d.content?.map(c=>c.text||"").join("\n")||"Sin respuesta");
    }catch(e){setAiMsg("⚠️ Error al conectar con la IA.");}
    setAiLoad(false);
  }
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'JetBrains Mono',monospace",color:C.text}}>
      ...
      <div style={{borderBottom:`1px solid ${C.border}`,background:C.surface,padding:"12px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {TABS.map(t=><button key={t.id} className="th" onClick={()=>setTab(t.id)} style={{background:tab===t.id?C.accent+"22":"transparent",color:tab===t.id?C.accent:C.muted,border:`1px solid ${tab===t.id?C.accent+"44":"transparent"}`,borderRadius:8,padding:"6px 12px",fontSize:12,cursor:"pointer",fontFamily:"inherit",transition:"all 0.2s"}}>{t.i} {t.l}</button>)}
          <button onClick={signOut} style={{background:"none",border:`1px solid ${C.border}`,color:C.muted,borderRadius:8,padding:"6px 12px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>Salir</button>
        </div>
      </div>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"24px 16px"}}>
        ...
        {tab==="dashboard"&&<div className="fa" style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12}}>
            ...
            <StatCard label="Psicología" value={ae+"/10"} sub={rp+"% reglas"} color={parseFloat(ae)>=7?C.green:C.yellow} icon="🧠"/>
          </div>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>{trades.slice(0,5).map(t=><div key={t.id} className="tr" style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:8,background:C.surface,border:`1px solid ${C.border}`}}>
              ...
              <div style={{marginLeft:"auto",display:"flex",gap:12,alignItems:"center"}}><span style={{color:C.muted,fontSize:11}}>{new Date(t.date).toLocaleDateString("es")}</span><span style={{color:t.pnl_usd>=0?C.green:C.red,fontWeight:700}}>{t.pnl_usd>=0?"+":""}${t.pnl_usd?.toFixed(0)}</span><span style={{color:C.muted,fontSize:12}}>{t.pnl_r>0?"+":""}{t.pnl_r}R</span></div>
            </div>)}</div>}
          </div>
        </div>}
        ...
        {tab==="trades"&&<div className="fa" style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>{ft.map(t=><div key={t.id} className="tr" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 18px",borderLeft:`3px solid ${t.pnl_usd>=0?C.green:C.red}`}}>
            <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <div style={{marginLeft:"auto",display:"flex",gap:12,alignItems:"center"}}>
                <div style={{textAlign:"right"}}><div style={{color:t.pnl_usd>=0?C.green:C.red,fontWeight:700,fontSize:16}}>{t.pnl_usd>=0?"+":""}${t.pnl_usd?.toFixed(2)}</div><div style={{color:C.muted,fontSize:11}}>{t.pnl_r>0?"+":""}{t.pnl_r}R</div></div>
                <div style={{display:"flex",gap:6}}><button className="be" onClick={()=>edit(t)}>Editar</button><button className="bd" onClick={()=>del(t.id)}>✕</button></div>
              </div>
            </div>
            ...
            <div style={{color:C.muted,fontSize:11,marginTop:6}}>{new Date(t.date).toLocaleString("es")} · E:{t.entry} · SL:{t.sl} · TP:{t.tp} · {t.risk_pct}% riesgo · 😐{t.emotion}/10 · {t.followed_rules?"✓":"✗"} reglas</div>
          </div>)}</div>}
        </div>}
        ...
        {tab==="stats"&&<div className="fa" style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}><BarChart label="P&L por Setup" data={ss.map(s=>({label:s.setup.substring(0,12),value:parseFloat(s.pnl.toFixed(1)),extra:s.wr+"%"}))}/></div>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}><BarChart label="Winrate por Día" data={ds}/></div>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}><BarChart label="P&L por Activo" data={as2}/></div>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}>
        ...
        {tab==="psychology"&&<div className="fa" style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12}}>
            ...
            <StatCard label="Siguió Reglas" value={rp+"%"} sub={ct.filter(t=>t.followed_rules).length+" de "+ct.length} color={parseFloat(rp)>=80?C.green:C.red} icon="📋"/>
        ...
        {tab==="ai"&&<div className="fa" style={{display:"flex",flexDirection:"column",gap:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><h2 style={{margin:0,fontSize:22,fontWeight:700}}>✦ IA Coach</h2><div style={{color:C.muted,fontSize:12}}>Análisis personalizado de tu rendimiento</div></div>
            <button className="bp" onClick={ai} disabled={aiLoad} style={{padding:"10px 22px",opacity:aiLoad?0.7:1}}>{aiLoad?"Analizando...":"▶ Analizar Ahora"}</button>
          </div>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:28,minHeight:300}}>
            ...
            {aiMsg&&!aiLoad&&<div><div style={{display:"flex",gap:10,alignItems:"center",marginBottom:20}}><div style={{background:"linear-gradient(135deg,#00d4ff,#b46ef5)",width:32,height:32,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center"}}>✦</div><div><div style={{fontWeight:700,fontSize:14}}>IA Coach</div><div style={{color:C.muted,fontSize:11}}>Basado en {ct.length} operaciones</div></div></div><div className="ap" style={{color:C.text}}>{aiMsg}</div></div>}
          </div>
        </div>}
      ...
      {modal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&setModal(false)}>
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:28,width:"100%",maxWidth:700,maxHeight:"90vh",overflowY:"auto",display:"flex",flexDirection:"column",gap:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:18,fontWeight:700}}>{editId?"Editar":"Nueva"} Operación</div><button onClick={()=>setModal(false)} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer"}}>✕</button></div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12}}>
            ...
            <Field label="Temporalidad"><select value={form.tf} onChange={e=>setForm(f=>({...f,tf:e.target.value}))} style={iS}>{TFS.map(t=><option key={t}>{t}</option>)}</select></Field>
          ...
          <div style={{background:C.surface,borderRadius:10,padding:16,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
            <Field label={"Emoción: "+form.emotion+"/10"}><input type="range" min="1" max="10" value={form.emotion} onChange={e=>setForm(f=>({...f,emotion:parseInt(e.target.value)}))} style={{width:"100%",accentColor:form.emotion>=7?C.green:form.emotion>=5?C.yellow:C.red}}/></Field>
            <Field label="¿Siguió Reglas?"><select value={form.followed_rules?"si":"no"} onChange={e=>setForm(f=>({...f,followed_rules:e.target.value==="si"}))} style={{...iS,color:form.followed_rules?C.green:C.red}}><option value="si">✓ Sí</option><option value="no">✗ No</option></select></Field>
          ...
          <Field label="Notas"><textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} rows={3} placeholder="Describe tu análisis..." style={{...iS,resize:"vertical"}}/></Field>
          <Field label="Imagen del Trade">
            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              <label style={{background:C.dim,border:`1px dashed ${C.border}`,borderRadius:8,padding:"10px 18px",cursor:"pointer",fontSize:12,color:C.muted}}>📎 Subir imagen<input type="file" accept="image/*" onChange={img} style={{display:"none"}}/></label>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
            ...
            <button className="bp" onClick={save} style={{padding:"10px 24px"}}>{editId?"Actualizar":"Guardar"} Operación</button>
          </div>
        </div>
      </div>}
    </div>
