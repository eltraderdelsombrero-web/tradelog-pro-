...
function EquityCurve({ trades, startBalance }) {
  ...
  useEffect(() => {
    ...
    [...trades].sort((a, x) => new Date(a.date) - new Date(x.date)).forEach(t => { b += parseFloat(t.pnl_usd) || 0; pts.push({ y: b }); });
...
function BarChart({ data, label }) {
  ...
  return <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
    {label && <div style={{ color:C.muted, fontSize:11, letterSpacing:2, textTransform:"uppercase", marginBottom:4 }}>{label}</div>}
    {data.map((d, i) => <div key={i} style={{ display:"flex", alignItems:"center", gap:8 }}>
      <div style={{ width:65, fontSize:11, color:C.muted, textAlign:"right", flexShrink:0 }}>{d.label}</div>
      ...
      <div style={{ width:45, fontSize:11, color:C.text, textAlign:"right" }}>{d.extra}</div>
    </div>)}
  </div>;
}
    ...
    <div style={{ minHeight:"100vh", background:C.bg, display:"flex", alignItems:"center", justifyContent:"center", padding:16, fontFamily:"'JetBrains Mono',monospace" }}>
      ...
      <div style={{ width:"100%", maxWidth:420, animation:"fi 0.4s ease" }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{ background:"linear-gradient(135deg,#00d4ff,#b46ef5)", width:60, height:60, borderRadius:18, display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:30, marginBottom:14 }}>◈</div>
          <div style={{ fontSize:26, fontWeight:700 }}>TradeLog <span style={{ color:C.accent }}>Pro</span></div>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:32 }}>
          <div style={{ fontSize:17, fontWeight:700, marginBottom:22, color:C.text }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <Field label="Contrasena"><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" style={iS} onKeyDown={e => e.key === "Enter" && go()} /></Field>
          </div>
          {err && <div style={{ background:C.red+"22", border:`1px solid ${C.red}44`, color:C.red, borderRadius:8, padding:"10px 14px", fontSize:12, marginTop:14 }}>{err}</div>}
          {ok && <div style={{ background:C.green+"22", border:`1px solid ${C.green}44`, color:C.green, borderRadius:8, padding:"10px 14px", fontSize:12, marginTop:14 }}>{ok}</div>}
          <button onClick={go} disabled={loading} style={{ width:"100%", marginTop:20, background:"linear-gradient(135deg,#00d4ff,#b46ef5)", color:"#000", border:"none", borderRadius:10, padding:"13px 0", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:loading ? 0.7 : 1 }}>
            {loading ? "Cargando..." : mode === "login" ? "Entrar" : "Crear Cuenta"}
          </button>
          <div style={{ marginTop:18, textAlign:"center" }}>
            <button onClick={() => { setMode(mode === "login" ? "register" : "login"); setErr(""); }} style={{ background:"none", border:"none", color:C.accent, cursor:"pointer", fontSize:12, fontFamily:"inherit" }}>
              {mode === "login" ? "No tienes cuenta? Registrate" : "Ya tienes cuenta? Inicia sesion"}
            </button>
          </div>
        </div>
        <div style={{ textAlign:"center", color:C.muted, fontSize:10, marginTop:16, letterSpacing:1 }}>DATOS SEGUROS Y CIFRADOS</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (token) {
      api.me().then(u => { setUser(u); setAl(false); }).catch(() => { clearToken(); setAl(false); });
    } else { setAl(false); }
  }, []);
  ...
  async function loadTrades() {
    setTl(true);
    try { const data = await api.getTrades(); setTrades(data); } catch (e) { console.error(e); }
    setTl(false);
  }

  function notify(msg, color = C.green) { setNotif({ msg, color }); setTimeout(() => setNotif(null), 3000); }
  function ef() { return { date: new Date().toISOString().slice(0, 16), asset:"EUR/USD", tf:"1h", type:"Buy", entry:"", sl:"", tp:"", pnl_r:"", pnl_usd:"", risk_pct:1, setup:"Breakout", market:"Trending alcista", notes:"", image:null, emotion:7, followed_rules:true, error:"Sin error" }; }

  async function saveTrade() {
    if (!form.asset || !form.date) return notify("Completa los campos", C.red);
    const t = { ...form, pnl_usd: parseFloat(form.pnl_usd) || 0, pnl_r: parseFloat(form.pnl_r) || 0, entry: parseFloat(form.entry) || null, sl: parseFloat(form.sl) || null, tp: parseFloat(form.tp) || null, risk_pct: parseFloat(form.risk_pct) || 1 };
    try {
      if (editId) { await api.updateTrade(editId, t); notify("Actualizado!"); }
      else { await api.addTrade(t); notify("Guardado!"); }
  ...
  async function delTrade(id) { try { await api.deleteTrade(id); await loadTrades(); notify("Eliminado", C.yellow); } catch (e) { notify("Error", C.red); } }
  ...
  async function signOut() { clearToken(); setUser(null); setTrades([]); setTab("dashboard"); }

  async function updateSettings(newSettings) {
    setUser(u => ({ ...u, ...newSettings }));
    clearTimeout(settingsTimer.current);
    settingsTimer.current = setTimeout(async () => {
      try { await api.updateSettings({ balance: newSettings.balance, risk_pct: newSettings.risk_pct }); } catch (e) { console.error(e); }
    }, 800);
  }

  const balance = parseFloat(user?.balance) || 10000;
  const filtered = applyTimeFilter(trades);
  const ct = filtered.filter(t => t.pnl_usd !== null);
  const wins = ct.filter(t => parseFloat(t.pnl_usd) > 0), losses = ct.filter(t => parseFloat(t.pnl_usd) < 0);
  const tPnl = ct.reduce((s, t) => s + (parseFloat(t.pnl_usd) || 0), 0);
  const totalPnl = trades.reduce((s, t) => s + (parseFloat(t.pnl_usd) || 0), 0);
  const bal = balance + totalPnl;
  const wr = ct.length ? (wins.length / ct.length * 100).toFixed(1) : "0.0";
  const gW = wins.reduce((s, t) => s + parseFloat(t.pnl_usd), 0), gL = Math.abs(losses.reduce((s, t) => s + parseFloat(t.pnl_usd), 0));
  ...
  let pk = balance, b2 = balance, mDD = 0;
  [...trades].sort((a, x) => new Date(a.date) - new Date(x.date)).forEach(t => { b2 += parseFloat(t.pnl_usd) || 0; if (b2 > pk) pk = b2; const d = ((pk - b2) / pk) * 100; if (d > mDD) mDD = d; });

  const setupStats = SETUPS.map(s => { const ts = ct.filter(t => t.setup === s); const w = ts.filter(t => parseFloat(t.pnl_usd) > 0).length; const p = ts.reduce((a, t) => a + parseFloat(t.pnl_usd), 0); return { setup:s, count:ts.length, wr:ts.length?(w/ts.length*100).toFixed(0):0, pnl:p }; }).filter(s => s.count > 0).sort((a, x) => x.pnl - a.pnl);
  const dayStats = Array(7).fill(null).map((_, i) => { const ts = ct.filter(t => new Date(t.date).getDay() === i); const w = ts.filter(t => parseFloat(t.pnl_usd) > 0).length; return { label:DAYS[i], value:ts.length?parseFloat((w/ts.length*100).toFixed(1)):0, extra:ts.length+"op" }; });
  const assetStats = ASSETS.map(a => { const ts = ct.filter(t => t.asset === a); const p = ts.reduce((s, t) => s + parseFloat(t.pnl_usd), 0); return { label:a, value:parseFloat(p.toFixed(1)), extra:ts.length>0?ts.length+"op":"" }; }).filter(a => a.extra !== "").sort((a, x) => x.value - a.value);
  const ae = ct.length ? (ct.reduce((s, t) => s + (parseInt(t.emotion) || 5), 0) / ct.length).toFixed(1) : "-";
  ...
  trades.forEach(t => {
    ...
    if (parseFloat(t.pnl_usd) > 0) monthlyData[key].wins++;
    if (parseFloat(t.pnl_usd) < 0) monthlyData[key].losses++;
    monthlyData[key].pnl += parseFloat(t.pnl_usd) || 0;
    monthlyData[key].pnlR += parseFloat(t.pnl_r) || 0;
    if (t.emotion) monthlyData[key].emotions.push(parseInt(t.emotion));
  });
  ...
  async function runAI() {
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000, system:"Eres un coach de trading profesional. Analiza los datos y da recomendaciones concretas en espanol. Usa emojis. Estructura: 1) Resumen 2) 3 fortalezas 3) 3 areas de mejora 4) Recomendacion psicologica 5) Plan para la proxima semana.", messages:[{role:"user",content:`Analiza mi bitacora:\n${JSON.stringify(sum,null,2)}`}] }) });
      const data = await res.json(); setAiMsg(data.content?.map(c => c.text || "").join("\n") || "Sin respuesta");
    } catch (e) { setAiMsg("Error al conectar con la IA."); }
    setAiLoad(false);
  }

  const TABS = [
    {id:"dashboard",l:"Dashboard",i:"◈"},{id:"trades",l:"Operaciones",i:"≡"},
    {id:"stats",l:"Estadisticas",i:"▲"},{id:"monthly",l:"Por Mes",i:"Cal"},
    {id:"report",l:"Reporte",i:"Rep"},{id:"psychology",l:"Psicologia",i:"◎"},
    {id:"ai",l:"IA Coach",i:"✦"},{id:"settings",l:"Config",i:"⚙"},
  ];
    ...
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:"'JetBrains Mono',monospace", color:C.text }}>
      ...
      <div style={{ borderBottom:`1px solid ${C.border}`, background:C.surface, padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100, flexWrap:"wrap", gap:8 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div><div style={{ fontWeight:700, fontSize:15 }}>TradeLog <span style={{ color:C.accent }}>Pro</span></div><div style={{ color:C.muted, fontSize:10, letterSpacing:2 }}>{user.email}</div></div>
        </div>
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"24px 16px" }}>
        ...
        {tab==="dashboard" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:20 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div><h2 style={{ margin:0, fontSize:22, fontWeight:700 }}>Dashboard</h2><div style={{ color:C.muted, fontSize:12 }}>{ct.length} ops {timeFilter!=="Todo"?"· "+timeFilter:""}</div></div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:12 }}>
            <StatCard label="Balance" value={"$"+bal.toLocaleString("en",{maximumFractionDigits:0})} sub={(totalPnl>=0?"+":"")+"$"+totalPnl.toFixed(0)+" P&L total"} color={totalPnl>=0?C.green:C.red} icon="💰"/>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
            ...
            <EquityCurve trades={ct} startBalance={balance}/>
          </div>
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>{trades.slice(0,5).map(t=><div key={t.id} className="tr" style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", borderRadius:8, background:C.surface, border:`1px solid ${C.border}` }}>
              <Badge text={t.type} color={t.type==="Buy"?C.green:C.red}/><span style={{ fontWeight:600, fontSize:13 }}>{t.asset}</span><span style={{ color:C.muted, fontSize:12 }}>{t.tf}</span><Badge text={t.setup} color={C.purple}/>
              <div style={{ marginLeft:"auto", display:"flex", gap:12, alignItems:"center" }}><span style={{ color:C.muted, fontSize:11 }}>{new Date(t.date).toLocaleDateString("es")}</span><span style={{ color:parseFloat(t.pnl_usd)>=0?C.green:C.red, fontWeight:700 }}>{parseFloat(t.pnl_usd)>=0?"+":""}${parseFloat(t.pnl_usd)?.toFixed(0)}</span><span style={{ color:C.muted, fontSize:12 }}>{parseFloat(t.pnl_r)>0?"+":""}{t.pnl_r}R</span></div>
            </div>)}</div>}
          </div>
        </div>}

        {tab==="trades" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
            <h2 style={{ margin:0, fontSize:22, fontWeight:700 }}>Operaciones</h2>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              <select value={timeFilter} onChange={e=>setTimeFilter(e.target.value)} style={{ ...iS, width:"auto", padding:"7px 12px" }}>{TIME_OPTIONS.map(o=><option key={o}>{o}</option>)}</select>
          ...
          {tl?<div style={{ color:C.muted }}>Cargando...</div>:ft.length===0?<div style={{ background:C.card, borderRadius:12, border:`1px solid ${C.border}`, padding:"60px 20px", textAlign:"center", color:C.muted }}>Sin operaciones</div>:
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>{ft.map(t=><div key={t.id} className="tr" style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 18px", borderLeft:`3px solid ${parseFloat(t.pnl_usd)>=0?C.green:C.red}` }}>
            <div style={{ display:"flex", gap:12, alignItems:"center", flexWrap:"wrap" }}>
              <Badge text={t.type} color={t.type==="Buy"?C.green:C.red}/><span style={{ fontWeight:700, fontSize:14 }}>{t.asset}</span><span style={{ color:C.muted, fontSize:12 }}>{t.tf}</span><Badge text={t.setup} color={C.purple}/><Badge text={t.market} color={C.yellow}/>
              <div style={{ marginLeft:"auto", display:"flex", gap:12, alignItems:"center" }}>
                <div style={{ textAlign:"right" }}><div style={{ color:parseFloat(t.pnl_usd)>=0?C.green:C.red, fontWeight:700, fontSize:16 }}>{parseFloat(t.pnl_usd)>=0?"+":""}${parseFloat(t.pnl_usd)?.toFixed(2)}</div><div style={{ color:C.muted, fontSize:11 }}>{parseFloat(t.pnl_r)>0?"+":""}{t.pnl_r}R</div></div>
                <div style={{ display:"flex", gap:6 }}><button className="be" onClick={()=>editTrade(t)}>Editar</button><button className="bd" onClick={()=>delTrade(t.id)}>X</button></div>
              </div>
            </div>
            ...
            <div style={{ color:C.muted, fontSize:11, marginTop:6 }}>{new Date(t.date).toLocaleString("es")} · E:{t.entry} · SL:{t.sl} · TP:{t.tp} · {t.risk_pct}% · {t.emotion}/10 · {t.followed_rules?"OK":"NO"}</div>
          </div>)}</div>}
        </div>}

        {tab==="stats" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:20 }}>
          <h2 style={{ margin:0, fontSize:22, fontWeight:700 }}>Estadisticas</h2>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}><BarChart label="P&L por Setup" data={setupStats.map(s=>({label:s.setup.substring(0,12),value:parseFloat(s.pnl.toFixed(1)),extra:s.wr+"%"}))}/></div>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}><BarChart label="Winrate por Dia" data={dayStats}/></div>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}><BarChart label="P&L por Activo" data={assetStats}/></div>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
              <div style={{ color:C.muted, fontSize:11, letterSpacing:2, textTransform:"uppercase", marginBottom:14 }}>Resumen Setups</div>
              {setupStats.length===0?<div style={{ color:C.muted }}>Sin datos</div>:setupStats.map(s=><div key={s.setup} style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:`1px solid ${C.border}`, fontSize:12 }}><span>{s.setup}</span><span style={{ color:C.muted }}>{s.count}op</span><span style={{ color:parseFloat(s.wr)>=50?C.green:C.red }}>{s.wr}%</span><span style={{ color:s.pnl>=0?C.green:C.red }}>{s.pnl>=0?"+":""}${s.pnl.toFixed(0)}</span></div>)}
            </div>
          </div>
        </div>}

        {tab==="monthly" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <h2 style={{ margin:0, fontSize:22, fontWeight:700 }}>Estadisticas por Mes</h2>
          {monthlyList.length===0?<div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:60, textAlign:"center", color:C.muted }}>Sin operaciones</div>:
          monthlyList.map(m=>{
            ...
            const topS=Object.entries(m.trades.reduce((acc,t)=>{acc[t.setup]=(acc[t.setup]||0)+1;return acc;},{})).sort((a,x)=>x[1]-a[1]).slice(0,4);
            return <div key={m.key} style={{ background:C.card, border:`1px solid ${C.border}`, borderLeft:`4px solid ${m.pnl>=0?C.green:C.red}`, borderRadius:12, padding:20 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
                <div><div style={{ fontWeight:700, fontSize:16, textTransform:"capitalize" }}>{m.label}</div><div style={{ color:C.muted, fontSize:12 }}>{m.trades.length} ops · {m.wins}W / {m.losses}L</div></div>
                <div style={{ display:"flex", gap:24 }}>
                  <div style={{ textAlign:"center" }}><div style={{ color:m.pnl>=0?C.green:C.red, fontWeight:700, fontSize:22 }}>{m.pnl>=0?"+":""}${m.pnl.toFixed(0)}</div><div style={{ color:C.muted, fontSize:10 }}>P&L</div></div>
                  <div style={{ textAlign:"center" }}><div style={{ color:parseFloat(wrM)>=50?C.green:C.red, fontWeight:700, fontSize:22 }}>{wrM}%</div><div style={{ color:C.muted, fontSize:10 }}>WINRATE</div></div>
                  <div style={{ textAlign:"center" }}><div style={{ color:parseFloat(avgR)>0?C.green:C.red, fontWeight:700, fontSize:22 }}>{parseFloat(avgR)>0?"+":""}{avgR}R</div><div style={{ color:C.muted, fontSize:10 }}>AVG R</div></div>
                  <div style={{ textAlign:"center" }}><div style={{ color:C.accent, fontWeight:700, fontSize:22 }}>{avgEmo}/10</div><div style={{ color:C.muted, fontSize:10 }}>EMOCION</div></div>
                </div>
              </div>
              <div style={{ marginTop:12, display:"flex", gap:6, flexWrap:"wrap" }}>{topS.map(([s,c])=><span key={s} style={{ background:C.purple+"22", color:C.purple, border:`1px solid ${C.purple}44`, borderRadius:4, padding:"2px 8px", fontSize:11 }}>{s} x{c}</span>)}</div>
            </div>;
          })}
        </div>}
        ...
        {tab==="report" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:20 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <select value={timeFilter} onChange={e=>setTimeFilter(e.target.value)} style={{ ...iS, width:"auto", padding:"7px 14px" }}>{TIME_OPTIONS.map(o=><option key={o}>{o}</option>)}</select>
          ...
          {(()=>{const trs=ft.filter(t=>t.pnl_usd!==null);const w=trs.filter(t=>parseFloat(t.pnl_usd)>0),l=trs.filter(t=>parseFloat(t.pnl_usd)<0);const p=trs.reduce((s,t)=>s+(parseFloat(t.pnl_usd)||0),0);const gw=w.reduce((s,t)=>s+parseFloat(t.pnl_usd),0),gl=Math.abs(l.reduce((s,t)=>s+parseFloat(t.pnl_usd),0));const pff=gl>0?(gw/gl).toFixed(2):gw>0?"inf":"0.00";const ar=trs.length?(trs.reduce((s,t)=>s+(parseFloat(t.pnl_r)||0),0)/trs.length).toFixed(2):"0.00";const wr2=trs.length?(w.length/trs.length*100).toFixed(1):"0.0";
          return <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10 }}>
            <StatCard label="Operaciones" value={trs.length} icon="📊"/><StatCard label="P&L" value={(p>=0?"+":"")+"$"+p.toFixed(0)} color={p>=0?C.green:C.red} icon="💰"/><StatCard label="Winrate" value={wr2+"%"} color={parseFloat(wr2)>=50?C.green:C.red} icon="🎯"/><StatCard label="Profit Factor" value={pff} color={parseFloat(pff)>=1.5?C.green:C.yellow} icon="⚡"/><StatCard label="Avg R" value={ar+"R"} color={parseFloat(ar)>0?C.green:C.red} icon="📈"/><StatCard label="Ganadas" value={w.length} color={C.green} icon="✅"/><StatCard label="Perdidas" value={l.length} color={C.red} icon="❌"/>
          </div>;})()}
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden" }}>
            <div style={{ padding:"14px 20px", borderBottom:`1px solid ${C.border}` }}><div style={{ color:C.muted, fontSize:11, letterSpacing:2, textTransform:"uppercase" }}>{ft.length} operaciones</div></div>
            {ft.length===0?<div style={{ padding:40, textAlign:"center", color:C.muted }}>Sin operaciones</div>:
            <div style={{ overflowX:"auto" }}><table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
              <thead><tr style={{ background:C.surface }}>{["Fecha","Activo","TF","Tipo","Setup","E","SL","TP","R","P&L","Riesgo","Emocion","Reglas","Error"].map(h=><th key={h} style={{ padding:"10px 14px", textAlign:"left", color:C.muted, fontWeight:600, whiteSpace:"nowrap", borderBottom:`1px solid ${C.border}` }}>{h}</th>)}</tr></thead>
              <tbody>{ft.map((t,i)=><tr key={t.id} className="tr" style={{ borderBottom:`1px solid ${C.border}`, background:i%2===0?"transparent":C.surface+"44" }}>
                ...
                <td style={{ padding:"10px 14px", color:parseFloat(t.pnl_r)>0?C.green:C.red, fontWeight:600 }}>{parseFloat(t.pnl_r)>0?"+":""}{t.pnl_r}R</td>
                <td style={{ padding:"10px 14px", color:parseFloat(t.pnl_usd)>=0?C.green:C.red, fontWeight:700 }}>{parseFloat(t.pnl_usd)>=0?"+":""}${parseFloat(t.pnl_usd)?.toFixed(0)}</td>
                <td style={{ padding:"10px 14px", color:C.muted }}>{t.risk_pct}%</td>
                <td style={{ padding:"10px 14px", color:parseInt(t.emotion)>=7?C.green:parseInt(t.emotion)>=5?C.yellow:C.red }}>{t.emotion}/10</td>
        ...
        {tab==="psychology" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
              ...
              {te.length===0?<div style={{ color:C.muted }}>Sin errores!</div>:te.map(([err,cnt])=><div key={err} style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:`1px solid ${C.border}` }}><span style={{ fontSize:13 }}>{err}</span><div style={{ display:"flex", alignItems:"center", gap:8 }}><div style={{ height:4, width:cnt*16, background:C.red, borderRadius:2 }}/><span style={{ color:C.red, fontWeight:700 }}>{cnt}</span></div></div>)}
            </div>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
              <div style={{ color:C.muted, fontSize:11, letterSpacing:2, textTransform:"uppercase", marginBottom:14 }}>Emocion vs Resultado</div>
              {[...Array(10)].map((_,i)=>{ const l=10-i; const ts=ct.filter(t=>parseInt(t.emotion)===l); const p=ts.reduce((s,t)=>s+parseFloat(t.pnl_usd),0); return ts.length>0?<div key={l} style={{ display:"flex", gap:10, alignItems:"center", marginBottom:6, fontSize:12 }}><span style={{ width:30, color:C.muted }}>Nv.{l}</span><div style={{ flex:1, height:14, background:C.border, borderRadius:2 }}><div style={{ height:"100%", width:`${(ts.length/ct.length)*100}%`, background:p>=0?C.green:C.red, borderRadius:2 }}/></div><span style={{ color:p>=0?C.green:C.red, width:55, textAlign:"right" }}>{p>=0?"+":""}${p.toFixed(0)}</span></div>:null; })}
            </div>
          </div>
        </div>}

        {tab==="ai" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:20 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div><h2 style={{ margin:0, fontSize:22, fontWeight:700 }}>IA Coach</h2><div style={{ color:C.muted, fontSize:12 }}>Analisis personalizado</div></div>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:28, minHeight:300 }}>
            {aiLoad&&<div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:200, gap:16 }}><div style={{ width:44, height:44, border:`3px solid ${C.border}`, borderTopColor:C.accent, borderRadius:"50%", animation:"sp 0.8s linear infinite" }}/><div style={{ color:C.muted }}>Analizando...</div></div>}
            {!aiLoad&&!aiMsg&&<div style={{ textAlign:"center", color:C.muted, paddingTop:60 }}><div style={{ fontSize:40, marginBottom:16 }}>✦</div><div>Haz clic en Analizar Ahora</div></div>}
            {aiMsg&&!aiLoad&&<div><div style={{ display:"flex", gap:10, alignItems:"center", marginBottom:20 }}><div style={{ background:"linear-gradient(135deg,#00d4ff,#b46ef5)", width:32, height:32, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>✦</div><div><div style={{ fontWeight:700 }}>IA Coach</div><div style={{ color:C.muted, fontSize:11 }}>{ct.length} operaciones</div></div></div><div className="ap" style={{ color:C.text }}>{aiMsg}</div></div>}
          </div>
        </div>}

        {tab==="settings" && <div className="fa" style={{ display:"flex", flexDirection:"column", gap:20, maxWidth:480 }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:24, display:"flex", flexDirection:"column", gap:16 }}>
            <div style={{ fontSize:13, color:C.text, padding:"8px 12px", background:C.surface, borderRadius:8 }}>Email: {user.email}</div>
            <Field label="Balance Inicial ($)">
              <input type="number" value={user.balance||10000} onChange={e=>updateSettings({...user,balance:parseFloat(e.target.value)||0})} style={iS}/>
            </Field>
            <Field label="Riesgo por defecto (%)">
              <input type="number" step="0.1" value={user.risk_pct||1} onChange={e=>updateSettings({...user,risk_pct:parseFloat(e.target.value)||1})} style={iS}/>
            </Field>
      ...
      {modal && <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }} onClick={e=>e.target===e.currentTarget&&setModal(false)}>
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:28, width:"100%", maxWidth:700, maxHeight:"90vh", overflowY:"auto", display:"flex", flexDirection:"column", gap:18 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}><div style={{ fontSize:18, fontWeight:700 }}>{editId?"Editar":"Nueva"} Operacion</div><button onClick={()=>setModal(false)} style={{ background:"none", border:"none", color:C.muted, fontSize:20, cursor:"pointer" }}>X</button></div>
          ...
          <Field label="Imagen del Trade">
            <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
              <label style={{ background:C.dim, border:`1px dashed ${C.border}`, borderRadius:8, padding:"10px 18px", cursor:"pointer", fontSize:12, color:C.muted }}>Subir imagen<input type="file" accept="image/*" onChange={handleImg} style={{display:"none"}}/></label>
